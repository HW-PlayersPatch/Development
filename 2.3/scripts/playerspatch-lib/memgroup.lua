dofilepath("data;scripts/playerspatch-lib/table_util.lia")
dofilepath("data:scripts/playerspatch-lib/helpers.lua")
...
MemGroup = {
	...
	_new = function(group_name, custom_attribs)
		...
		-- getter
		function new_group:GetEntities(filter)
			if (filter == nil) then
				return self._entities
			end
			return filter(self._entities, filter)
		end
		return new_group
	end,
	-- Create
...
GLOBAL_SHIPS = MemGroup:create("GLOBAL_SHIPS")